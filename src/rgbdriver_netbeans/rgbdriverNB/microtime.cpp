...
microTime::microTime(unsigned int years, byte months, byte days, byte hours, byte minutes, byte seconds) {
  this->timeZone = 0;
  this->years = constrain(years, (uint)1970, (uint)9999);
  this->months = constrain(months, (byte)1, (byte)12);
  ...
  this->days = constrain(days, (byte)1, currentCascadeDay);
  this->hours = constrain(hours, (byte)0 , (byte)(CASCADE_HOUR -1));
  this->minutes = constrain(minutes, (byte)0 , (byte)(CASCADE_MINUTE -1));
  this->seconds = constrain(seconds, (byte)0 , (byte)(CASCADE_SECOND -1));
  this->microSeconds = 0;
  this->timeSet = true;
}
...
microTime::microTime(unsigned int years, byte months, byte days, byte hours, byte minutes, byte seconds, unsigned long microSeconds) {
    ...
    this->minutes = constrain(minutes, (byte)0 , (byte)(CASCADE_MINUTE -1));
    this->seconds = constrain(seconds, (byte)0 , (byte)(CASCADE_SECOND -1));
    this->microSeconds = constrain(microSeconds, 0UL, CASCADE_MICRO -1UL);
    this->timeSet = true;
}
...
//  Serial.print(" < ");
//  Serial.println(rhs.year());
//  Serial.print(this->month());
//  Serial.print(" < ");
//  Serial.println(rhs.month());
//  Serial.print(this->day());
//  Serial.print(" < ");
//  Serial.println(rhs.day());
//  Serial.print(this->hour());
//  Serial.print(" < ");
//  Serial.println(rhs.hour());
//  Serial.print(this->minute());
//  Serial.print(" < ");
//  Serial.println(rhs.minute());
//  Serial.print(this->second());
//  Serial.print(" < ");
//  Serial.println(rhs.second());
  ...
  else if(this->microSecond() > rhs.microSecond()){
    return false;
  }
}
...
void microTime::operator+=(unsigned long seconds) {
  // Serial.println("microTime += operation: ");
  // Serial.println((int)floor(seconds / 60.0));
  
  for(int i = 0; i < (int)floor(seconds / 60.0); i++) {
    // Serial.print("adding 60 seconds ");
    this->seconds += 60;
    cascadeSecond();
    cascadeMinute();
    cascadeHour();
...
void microTime::setTimeZone(char timeZone) {
  this->timeZone = constrain((int)timeZone, -12, 14);
}
